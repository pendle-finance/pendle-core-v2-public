...
import {IPYieldToken, PYIndex, PYIndexLib} from "../core/StandardizedYield/PYIndex.sol";
...
contract ActionSimple is ActionBase, IPActionSimple {
    ...
    /// @dev The interface of this simple function is subject to change without notice.
    function swapExactSyForPtSimple(address receiver, address market, uint256 exactSyIn, uint256 minPtOut)
        external
    ...
    /// @dev The interface of this simple function is subject to change without notice.
    function swapExactTokenForYtSimple(address receiver, address market, uint256 minYtOut, TokenInput calldata input)
        external
        payable
    ...
    /// @dev The interface of this simple function is subject to change without notice.
    function swapExactSyForYtSimple(address receiver, address market, uint256 exactSyIn, uint256 minYtOut)
        external
    ...
    /// @dev The interface of this simple function is subject to change without notice.
    function addLiquiditySinglePtSimple(address receiver, address market, uint256 netPtIn, uint256 minLpOut)
        external
        returns (uint256 netLpOut, uint256 netSyFee)
    {
        ...
        (uint256 netPtSwapMarket,,,) = _readMarket(market)
            .approxSwapPtToAddLiquidityOnchain(YT.newIndex(), netPtLeft, netSyReceived, block.timestamp);
    ...
    /// @dev The interface of this simple function is subject to change without notice.
    function addLiquiditySingleSySimple(address receiver, address market, uint256 netSyIn, uint256 minLpOut)
        external
        returns (uint256 netLpOut, uint256 netSyFee)
    {
        (IStandardizedYield SY,, IPYieldToken YT) = IPMarket(market).readTokens();
    ...
    function _addLiquiditySingleSySimple(
        ...
        (uint256 netPtOutMarket,,,) = _readMarket(market)
            .approxSwapSyToAddLiquidityOnchain(YT.newIndex(), netSyLeft, netPtReceived, block.timestamp);
    ...
    /// @dev The interface of this simple function is subject to change without notice.
    function removeLiquiditySinglePtSimple(address receiver, address market, uint256 netLpToRemove, uint256 minPtOut)
        external
        returns (uint256 netPtOut, uint256 netSyFee)
    {
        uint256 netSyLeft;
        ...
        (uint256 netSyOutBurn, uint256 netPtOutBurn) =
            IPMarket(market).burn(_entry_swapExactSyForPt(market, false), receiver, netLpToRemove);
    ...
    function _swapExactSyForPtSimple(address receiver, address market, uint256 exactSyIn, uint256 minPtOut)
        internal
        returns (uint256 netPtOut, uint256 netSyFee)
    {
        ...
        (uint256 netPtOutMarket,,) =
            _readMarket(market).approxSwapExactSyForPtOnchain(YT.newIndex(), netSyLeft, block.timestamp);
    ...
    function _swapExactSyForYtSimple(
        ...
        (uint256 netYtOutMarket,,) =
            _readMarket(market).approxSwapExactSyForYtOnchain(YT.newIndex(), netSyLeft, block.timestamp);
