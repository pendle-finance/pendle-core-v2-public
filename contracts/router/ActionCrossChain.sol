...
import {TokenOutput, IPActionCrossChain} from "../interfaces/IPActionCrossChain.sol";
import {IPFixedPricePTAMM} from "../interfaces/IPFixedPricePTAMM.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {SwapType, IPSwapAggregator} from "./swap-aggregator/IPSwapAggregator.sol";
...
contract ActionCrossChain is ActionBase, IPActionCrossChain {
    function swapWithFixedPricePTAMM(
        ...
        uint256 tokenOut = IPFixedPricePTAMM(fixedPricePTAMM).swapExactPtForToken(
            __swapDestination(receiver, out),
            PT,
            exactPtIn,
            out.tokenRedeemSy,
            EMPTY_BYTES
    ...
    /// @dev The caller must ensure that the address returned by `__swapDestination(...)` has `netTokenRedeemSy` amount of `out.tokenRedeemSy` tokens before calling this function.
    function __handleTokenOutput(
        address receiver,
        uint256 netTokenRedeemSy,
        TokenOutput calldata out
    ) internal returns (uint256 netTokenOut) {
        SwapType swapType = out.swapData.swapType;
        if (swapType == SwapType.NONE) netTokenOut = netTokenRedeemSy;
