...
import "./StorageLayoutLe1.sol";

contract ActionMarketCoreStaticLe1 is StorageLayoutLe1, IPActionMarketCoreStaticLe1 {
    ...
    using MarketApproxPtInLibLe1 for MarketState;
    using MarketApproxPtOutLibLe1 for MarketState;
    ...
    function _calcPriceImpactPY(address market, int256 netPtOut) internal view returns (uint256) {
        return IPRouterStaticLe1(address(this)).calcPriceImpactPY(market, netPtOut);
    }

    function _calcPriceImpactPt(address market, int256 netPtOut) internal view returns (uint256) {
        return IPRouterStaticLe1(address(this)).calcPriceImpactPt(market, netPtOut);
    }

    function _calcPriceImpactYt(address market, int256 netPtOut) internal view returns (uint256) {
        return IPRouterStaticLe1(address(this)).calcPriceImpactYt(market, netPtOut);
    }
    ...
    function _mintSyFromTokenStatic(
        ...
        return
            IPRouterStaticLe1(address(this)).mintSyFromTokenStatic(
                _getSyMarket(market),
                tokenIn,
                netTokenToDeposit,
                bulk
    ...
    function _redeemSyToTokenStatic(
        ...
        return
            IPRouterStaticLe1(address(this)).redeemSyToTokenStatic(_getSyMarket(market), tokenOut, netSyToRedeem, bulk);
    }

    function _getTradeExchangeRateExcludeFee(address market, MarketState memory state) internal view returns (uint256) {
        return IPRouterStaticLe1(address(this)).getTradeExchangeRateExcludeFee(market, state);
    }
    ...
    function _pyIndex(address market) private view returns (PYIndex) {
        return PYIndex.wrap(IPRouterStaticLe1(address(this)).pyIndexCurrentViewMarket(market));
    }
